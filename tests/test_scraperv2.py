"""
test_scraperv2.py
Tests for EmagCrawlerV2 and FilterCatalog.
Run alongside test_scraper.py — both share pytest.ini (asyncio_mode=auto).
"""
...
def catalog_with_data(tmp_path) -> FilterCatalog:
    ...
    data = {
        "models": {
            "galaxy a26":       "model-f9396,galaxy-a26-v-11000001",
            "galaxy a37":       "model-f9396,galaxy-a37-v-11000002",
            "galaxy a57":       "model-f9396,galaxy-a57-v-11000003",
            "galaxy s25":       "model-f9396,galaxy-s25-v-11000004",
        ...
        "storage": {
            ...
            "512gb": "memorie-interna-f9441,512-gb-v30058",
        },
    }
...
async def crawler_v2(tmp_path):
    cache = tmp_path / "emag_filters.json"
    return EmagCrawlerV2(
        filter_cache=cache,
        headless=True,
        max_retries=1,
        timeout_ms=8_000,
...
def test_catalog_token_subset_matching(catalog_with_data):
    ...
    # Query has extra tokens (5G). Matches 's26' & 'plus'. Should return 's26 plus', not 's26'
    slug_plus = catalog_with_data.get_model_slug("Samsung S26 Plus 5G")
    assert slug_plus == "model-f9396,galaxy-s26-plus-v-14963102"

    # Query tokens: 's26'. Should strictly match 'galaxy s26', not 'galaxy s26 plus'
...
def test_catalog_storage_exact(catalog_with_data):
    ...
    assert catalog_with_data.get_storage_slug("512GB") == "memorie-interna-f9441,512-gb-v30058"
...
def test_build_filtered_url_contains_slugs(crawler_v2):
    url = crawler_v2._build_filtered_url(
    ...
    assert url.startswith("https://www.emag.ro/search/telefoane-mobile/brand/samsung")
...
@pytest.mark.asyncio
async def test_v2_update_json(crawler_v2, sample_json):
    updated = await crawler_v2.update_json_file(sample_json)
...
@pytest.mark.asyncio
async def test_v2_update_json_preserves_subsidy(crawler_v2, sample_json):
    updated = await crawler_v2.update_json_file(sample_json)
